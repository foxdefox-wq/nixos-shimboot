...
- Debian Unstable (Sid)
- Alpine Linux
- NixOS (requires Nix installed on the build host)
...
NixOS is also supported. Requires Nix to be installed on the build machine. Pass `distro=nixos` to use it:
```bash
sudo ./build_complete.sh dedede distro=nixos
```

You can change the desktop environment with the `desktop` argument (same options as Debian):
```bash
